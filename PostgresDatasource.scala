...
case class ConnectionProperties(url: String, user: String, password: String, tableName: String, numPartitions: String)
...
class PostgresScanBuilder(options: CaseInsensitiveStringMap) extends ScanBuilder {
  override def build(): Scan = new PostgresScan(ConnectionProperties(
    options.get("url"), options.get("user"), options.get("password"), options.get("tableName"), options.get("numPartitions")
  ))
}
...
class PostgresScan(connectionProperties: ConnectionProperties) extends Scan with Batch {
  ...
  override def planInputPartitions(): Array[InputPartition] = Array(
    new PostgresPartition(0,4), new PostgresPartition(5,9), new PostgresPartition(10,14))
